...
import 'package:flutter_map_lab/network.dart';
...
class MapSampleState extends State<MapSample> {
  ...
  static final CameraPosition _kHome = CameraPosition(
    target: LatLng(7.174887426444304, 100.61313311111138),
    zoom: 14.4746,
  );

  static final CameraPosition _kStf = CameraPosition(
  ...
  Widget build(BuildContext context) {
    ...
    return new Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: _goToTheStf,
        label: Text('To the Science and Technology Faculty!'),
        icon: Icon(Icons.directions_boat),
      ),
    );
  }

  Future<void> _goToTheStf() async {
    final GoogleMapController controller = await _controller.future;
    controller.animateCamera(CameraUpdate.newCameraPosition(_kStf));
  }
